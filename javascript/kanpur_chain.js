...
locateBtn.addEventListener("click", function () {
  ...
  navigator.geolocation.getCurrentPosition(function (position) {
    ...
    const coords = [lon, lat];   // EPSG:4326 → lon,lat
...
document.getElementById("submitUniqueId").addEventListener("click", async () => {
  ...
  try {
    ...
    if (chainageYes && chainageYes.checked) {
      ...
      // reload geometry
      const resGeo = await fetch(`http://localhost:8880/Kanpur/patch/geometry/${selectedRoadId}`);
...
// Start and End marker sources
var startMarkerSource = new ol.source.Vector();
var endMarkerSource = new ol.source.Vector();
