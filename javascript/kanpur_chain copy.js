...
document.getElementById("ShowRoads").addEventListener("click", function () {
  // Toggle visibility
  if (knp_road.getVisible()) {
    // If the road is currently visible, hide it
    knp_road.setVisible(false);
  } else {
    // If the road is currently hidden, show it
    knp_road.setVisible(true);
  }
});
...
// const titleFromUrl = urlParams.get("title"); // e.g. "Zone-4"

// console.log("Project:", projectIdFromUrl, "User:", userIdFromUrl, "Title:", titleFromUrl);

// // Extract zone number from title
// let zoneFromUrl = null;
// if (titleFromUrl) {
//   const match = titleFromUrl.match(/Zone[-\s]?(\d+)/i);
//   if (match) zoneFromUrl = match[1];
// }
...
//   const match = titleFromUrl.match(/Zone[-\s]?(\d+)/i);
//   if (match) {
//     zoneNo = match[1]; // "Zone-2" -> "2"
//   }
// }

// if (!zoneNo) {
//   alert("Zone parsing failed");
//   throw new Error("Zone parsing failed");
// }

// // --- Predefined bounding boxes for WMS layer zoom ---
// const zoneBounds = {
//   'Zone-2': [80.308349609375, 26.337308883666992, 80.4718246459961, 26.442121505737305], // Bounding box for Zone-2
//   // Add other zones as needed
// };

// // Get the bounding box for the selected zone
// const bounds = zoneBounds[`Zone-${zoneNo}`];

// // Initialize the map


// // --- WMS Layer Setup ---
// const knp_road = new ol.layer.Image({
//   title: `Chainage Zone ${zoneNo}`,
//   visible: true,
//   source: new ol.source.ImageWMS({
//     url: `${GEOSERVER_BASE_URL1}/wms?`,
//     crossOrigin: "anonymous",
//     params: {
//       'LAYERS': 'Lko_Summary:segmentszone2roads',
//       'CQL_FILTER': `zone_no = 'Zone-${zoneNo}'`,
//     },
//     ratio: 1,
//     serverType: "geoserver"
//   })
...
// // Zoom to the bounding box if available
// if (bounds) {
//   map.getView().fit(bounds, { padding: [20, 20], duration: 1000 });
// } else {
//   alert("No bounding box found for this zone.");
// }
// //----------------------------------- Zone Roads Fetching and Display ------------------------------------//

// // Validate that zone is provided
// if (!titleFromUrl) {
//   alert("Invalid zone in URL");
//   throw new Error("Zone parsing failed");
// }

// // Extract zone number from title (e.g., Zone-4 -> 4)
// let zoneNo = null;
// const match = titleFromUrl.match(/Zone[-\s]?(\d+)/i);
// if (match) {
//   zoneNo = Number(match[1]); // "Zone-4" -> 4
// }

// // If zoneNo is not parsed correctly, alert user
// if (!zoneNo) {
//   alert("Invalid zone format in URL");
//   throw new Error("Zone parsing failed");
// }


// // Fetch roads based on zone number (zone_no)
// async function loadZoneRoads() {
//       const res = await fetch(`http://localhost:8800/Kanpur/roads-by-zone?zoneNo=2`);
//       if (!res.ok) throw new Error("Failed to load roads");
//       return await res.json();
//     }

//     // Show roads on the map using OpenLayers
//     async function showZoneRoadsOnMap() {
//       const roads = await loadZoneRoads();

//       if (!roads.length) {
//         alert("No roads found for zone 4");
//         return;
//       }
...
//       roads.forEach(r => {
//         const geojson = JSON.parse(r.geom);
// console.log("GeoJSON:", geojson);  // Check if it's a valid GeoJSON


//         const vectorSource = new ol.source.Vector({
//           features: new ol.format.GeoJSON().readFeatures(geojson, {
//             featureProjection: 'EPSG:4326' // Ensure the projection is correct for OpenLayers
//           })
//         });
...
//       map.getView().fit(extent, { padding: [20, 20], duration: 1000 });
//     }

//     // Call the function to display roads on the map
//     showZoneRoadsOnMap();
...
map.on("singleclick", async function (evt) {
  ...
  try {
    ...
    zoomToRoadExtent(selectedRoadId);
...
//   console.log("Submitting updated patch data:", updatedData);

//   await savePatchData(updatedData);

//   // reset state after successful save
//   patchTableDirty = false;
//   disablePatchTableEdit();
//   document.getElementById("savePatchBtn").classList.add("hidden");
// });
